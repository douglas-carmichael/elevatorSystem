...
extension DCLEngine {
    func helpText(topic: String?) -> String {
        ...
        s += "  INSTALL       LOGOUT        LPDCP         MAIL          MONITOR\n"
        s += "  MOUNT         PHONE         PRINT         PRODUCT       PURGE\n"
        s += "  RECALL        RENAME        REPLY         REQUEST       RUN\n"
        s += "  SCRIPTING     SEARCH        SET           SHOW          SPAWN\n"
        s += "  STOP          STORAGE       SUBMIT        TUTORIAL      TYPE\n"
        s += "  WAIT          WRITE\n\n"
    ...
    func helpTopic(_ raw: String) -> String {
        ...
        case matches(t, "SHOW"):
            ...
            s += "      ALARMS           DIAGNOSTICS    LOGICAL [/PROC]\n"
            s += "      (Elevator subjects -- CAB, BUILDING, DISPATCH, CALLS, LOAD --\n"
            s += "       moved to the LPDCP layered product; HELP LPDCP for details.)\n"
        ...
        case matches(t, "SET"):
            ...
            s += "  Cab and building subjects (SET CAB, SET BUILDING) live\n"
            s += "  under the LPDCP layered product -- HELP LPDCP.\n"
            return s
        case matches(t, "CALL"):
            ...
            s += "      world-level hall call (LPDCP SHOW CALLS) and allocated to the\n"
            ...
            s += "      Requires  LPDCP SET BUILDING /DISPATCH=DESTINATION  for the\n"
            s += "      auto-driver to stand down and let the allocator manage every trip.\n"
            return s
        case matches(t, "OPEN"):
        ...
        case matches(t, "ELEVATOR"):
            var s = "\n  Elevator demo flow:\n"
            s += "    LPDCP SET CAB L02 /MANUAL    ! Disable auto-driver for cab L02\n"
            ...
            s += "    LPDCP SET CAB L02 /AUTOMATIC ! Hand control back to the auto-driver\n"
            s += "    LPDCP SET CAB L02 /FREIGHT   ! Designate as freight cab\n"
            s += "    LPDCP SET CAB L02 /PAX       ! Designate as passenger cab\n\n"
            s += "  See HELP LPDCP for the full LPD Elevator Control Program reference.\n"
            return s
        case matches(t, "LPDCP", min: 5):
            var s = "\n  LPDCP <subverb> <noun> [args] [/qualifiers]\n"
            s += "      LPD Elevator Control Program -- the layered product that owns\n"
            s += "      every elevator-specific SET / SHOW subject. Mirrors the real-\n"
            s += "      VMS layered-tool pattern (NCP for DECnet, LATCP for LAT, ...).\n\n"
            s += "  Subverbs:\n"
            s += "      SHOW    Display elevator state\n"
            s += "      SET     Modify elevator state\n"
            s += "      HELP    Detailed command reference (LPDCP HELP)\n\n"
            s += "  SHOW subjects:\n"
            s += "      CAB [label]    Per-cab status sheet (or list-all when no label).\n"
            s += "      BUILDING       Safety mode + dispatch + cab census summary.\n"
            s += "      DISPATCH       Group dispatch mode and recent allocations.\n"
            s += "      CALLS          Latched hall calls + per-cab car-call queues.\n"
            s += "      LOAD           Per-cab platform load-cell readouts.\n\n"
            s += "  SET subjects:\n"
            s += "      CAB <label>    /MANUAL | /AUTOMATIC | /PAX | /FREIGHT\n"
            s += "                     /PHASE2=ON|OFF | /INDEPENDENT=ON|OFF | /LOAD=<kg>\n"
            s += "      BUILDING       /FIRE_RECALL=ON|OFF [/FLOOR=n]\n"
            s += "                     /EPO=ON|OFF [/CAB=<label>]\n"
            s += "                     /NORMAL\n"
            s += "                     /DISPATCH=COLLECTIVE|DESTINATION\n\n"
            s += "  SYS$LOGIN:LOGIN.COM defines foreign-command aliases CAB, BLDG,\n"
            s += "  DPATCH, CALLS, LOAD, FIRE and NORMAL so a returning operator can\n"
            s += "  type the short form, e.g.  $ CAB L01  expands to  LPDCP SHOW CAB L01.\n"
            return s
